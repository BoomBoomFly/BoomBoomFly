...
| `/fmu/in/vehicle_visual_odometry` | `/vision_to_dds_node` | PX4 uXRCE-DDS reader / EKF2 | ROS → PX4 | `px4_msgs/msg/VehicleOdometry` | `UNIT_TESTED` bridge contract; end-to-end `UNVERIFIED` | TF-stamp-only output time; 200 ms age, 20 ms future skew, 500 ms jump, quality/epoch/freeze and manual-reset gates | only `/vision_to_dds_node` | T265 adapter supplies measured health; PX4 endpoint/EKF2 remain formal-SITL work | `READY_FOR_FORMAL_SITL` |
